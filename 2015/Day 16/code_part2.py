...
def CheckQuality(quality):
    ...
    for x in sues.viablelist:
        ...
        if value != None:
            if value == elements[1] and elements[0] not in ['cats','trees','pomeranians','goldfish']:
                newviabledict[x] = sues.viablelist[x]
            if int(value) >= int(elements[1]) and elements[0] in ['cats','trees']:
                newviabledict[x] = sues.viablelist[x]
            if int(value) < int(elements[1]) and elements[0] in ['pomeranians','goldfish']:
                newviabledict[x] = sues.viablelist[x]
